...
PUBLIC void out_char(CONSOLE* p_con, char ch)
{
	...
	case '\t':
		if (p_con->cursor > p_con->original_addr) {
			p_con->cursor = ((p_con->cursor % SCREEN_WIDTH) / 6 + 1) * 6 + p_con->cursor / SCREEN_WIDTH * SCREEN_WIDTH;
		}
		break;
	default:
...
PUBLIC void clear(TTY* p_tty){
	t_8* tw;
	for(tw = (t_8*)(V_MEM_BASE + 2 * p_tty->p_console->original_addr); tw < (t_8*)(V_MEM_BASE + 2 * p_tty->p_console->original_addr + 2 * p_tty->p_console->v_mem_limit); tw += 2){
		*tw=0;
	}
	init_screen(p_tty);
}
